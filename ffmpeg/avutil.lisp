(load (cffi-grovel:process-grovel-file "ffmpeg/avutil-grovel.lisp" "/dev/shm/_ffmpeg.o"))
...
(defun ffmpeg-dict-from-list (kv-list)
  (let ((ffmpeg-dict (make-ffmpeg-dict)))
    (dolist (pair kv-list)
      (setf (ffmpeg-dict-ref ffmpeg-dict (car pair)) (cdr pair)))
    ffmpeg-dict))
